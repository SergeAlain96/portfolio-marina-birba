-- OBSOLETE (parties policies) : ne pas reappliquer apres migration_admin_permissions.sql
-- La policy "auth write project_documents" ci-dessous utilise auth.role() = 'authenticated',
-- ce qui autoriserait tout compte connecte a ecrire dans la table.
-- La policy appliquee est "admin manage project_documents"
-- (basee sur public.is_portfolio_admin()), definie dans migration_admin_permissions.sql.
--
