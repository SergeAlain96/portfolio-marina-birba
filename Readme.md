...
* Importer, remplacer ou supprimer une image
...
Pour une installation neuve, exécuter `supabase/schema.sql` puis `supabase/storage.sql`.

Pour une base existante, exécuter les scripts suivants dans cet ordre :
...
Après cette modification, se déconnecter puis se reconnecter pour actualiser le jeton d'authentification. Les services publics restent limités aux lignes dont le statut est `published`.
